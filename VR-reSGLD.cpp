...
double swap_rate(double sum1, double sum2, double temp1, double temp2, int N, int n, double sigma, double F)
{
    ...
    if (energy_diff - bias_term < 0.0) 
    {
        exponent = temp_diff * (ratio * energy_diff - bias_term) ;
    } else
    {
        exponent = 0.0 ;
    }
...
int main() {
    ...
    for (int k = 1; k < k_max; ++k) {
        ...
        if(k % 200 == 0)
        {
            ...
            double sigma_hat = (1 - smooth_factor) * sigma_hat + smooth_factor * (sigma1 + sigma2) / 2 ;
        ...
        for (int b = 0; b < n; b++)
        {
            energy_sum1 += energy(&pSample_Set(b), beta1) - energy(&pSample_Set(b), beta1_VR) ;

            energy_sum2 += energy(&pSample_Set(b), beta2) - energy(&pSample_Set(b), beta2_VR) ;
        }
